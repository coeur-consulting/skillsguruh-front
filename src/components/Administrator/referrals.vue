...
export default {
  data() {
    return {
      email: "",
      message: `https://skillsguruh.herokuapp.com/register/?referral_code=${this.$store.getters.admin.referral}`,
    };
  },
  mounted() {
    this.message = `https://skillsguruh.herokuapp.com/register/?referral_code=${this.$store.getters.admin.referral}`;
  },
  methods: {
    sendInvite() {
      var data = {
        email: this.email,
        code: this.$store.getters.admin.referral,
      };
      this.$http
