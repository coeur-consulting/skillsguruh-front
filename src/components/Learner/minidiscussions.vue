    ...
    <div class="shadow-sm bg-white py-4 rounded">
      ...
      <div class="main_content" v-if="filterDiscusions.length">
        <div
          ...
          v-for="(item, index) in filterDiscusions"
          :key="index"
        >
          <div class="top_dis d-flex align-items-center mb-2 position-relative">
            <b-dropdown
              size="sm"
              variant="transparent"
              no-caret
...
export default {
  ...
  computed: {
    ...
    mostanswers() {
      var val = this.filterDiscusions.slice(0).sort((a, b) => {
  ...
  methods: {
    ...
    getdiscussions() {
      this.$http
        ...
        .then((res) => {
          if (res.status == 200) {
            this.recentdiscussions = res.data.slice(0, 6);
            this.toggleData();
          }
        })
        .catch((err) => {
